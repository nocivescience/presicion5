...
// funciones para visores
let colorDefault= 'rgb(74, 74, 74)';
const visorUno = document.getElementById("visor-uno");
const visorDos = document.getElementById("visor-dos");
const visorTres = document.getElementById("visor-tres");
function mouseStart(title, visor, startX, startY, rotX, rotY, background= colorDefault){
  visor.innerHTML = `
...
window.addEventListener("mousemove", function (event) {
  if (gameInProgress) {
    ...
    let colorDefault1, colorDefault2, colorDefault3;
    if (Math.abs(mouseDeltaX) <2){
      colorDefault1 = 'rgb(217, 159, 1)';
    }else{
      colorDefault1 = 'rgb(74, 74, 74)';
    }
    mouseStart('Deltas',visorUno, mouseDeltaX, mouseDeltaY, rotationX, rotationY, colorDefault1);
    ...
    if(accelerationY<.4&&accelerationY>.1){
      colorDefault2 = 'rgb(1, 109, 217)';
    }else{
      colorDefault2 = 'rgb(74, 74, 74)';
    }
    mouseStart('Acceleration', visorDos, accelerationX.toFixed(4), accelerationY.toFixed(4), rotationX, rotationY, colorDefault2);
    frictionX = gravity * Math.cos((rotationY / 180) * Math.PI) * friction;
    frictionY = gravity * Math.cos((rotationX / 180) * Math.PI) * friction;
    if(frictionX>.0196&&frictionX<.0197||frictionY>.0196&&frictionY<.0197){
      colorDefault3 = 'rgb(1, 217, 1)';
    }else{
      colorDefault3 = 'rgb(74, 74, 74)';
    }
    mouseStart('Friction' , visorTres, frictionX.toFixed(4), frictionY.toFixed(4), rotationX, rotationY, colorDefault3);
  }
});
